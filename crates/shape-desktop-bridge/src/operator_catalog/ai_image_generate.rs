...
pub(crate) fn configuration_for_ai_image_generate(
    ...
    validate_parameters_or_empty(instruction, output, &[])?;
...
fn decode(
    ...
    if wire.candidate_count != 1 {
        return Err(
            "the first AI image desktop consumer requires exactly one Candidate".to_owned(),
        );
    }
    validate_parameters_or_empty(&wire.instruction, wire.output, &wire.constraints)?;
    Ok(wire)
}
...
fn validate_parameters_or_empty(
    ...
    AiImageGenerateParameters::new(validation_instruction, output, 1, constraints.to_vec())
        .map(|_| ())
        .map_err(|error| error.to_string())
}
