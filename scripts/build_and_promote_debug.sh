...
# Build the complete user-launchable Shape debug product, validate its packaged
# startup path, and atomically advance the stable current-debug application.
set -eu
...
Builds Shape.app in one stable external candidate directory, runs repository
and packaged desktop checks, then atomically promotes the bundle to current-debug.
