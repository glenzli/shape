...
新建文本直接进入统一工作区，先选择普通文本或口播脚本，再描述需求或手写。
听力练习、口播、对话是写作预设，使用同一种口播脚本格式。规则与示例可在格式面板中查看和复制。
语气、读者与文风是可展开的创作设置；个人预设的含义复制进节点，不依赖后来修改的个人库。

对一份原稿添加文本编辑会创建独立输出。再次添加编辑节点可以建立另一个分支；打开已有节点继续修改它。
没有已采用原稿时不提供文本编辑动作。原稿更新后，已有编辑和配音保持固定输入版本并提示更新；
用户点击“使用最新原稿”后才切换输入。旧候选不能越过输入版本、输出版本或规则变化的检查。

普通文本与口播脚本的解释由 `TextDocumentContract` 随不可变 Revision 保存。
口播脚本格式版本为 `20260922.2`，具体语法见 [脚本规则](SPEECH_SCRIPT.md)。
写作预设决定内容安排，语法决定指令如何解释，配音配置决定角色使用什么声音。
...
Shape 明确指定 `codex_gpt_5_6_luna`，不回退到其他模型；模型未就绪时会保留草稿并报告失败。
