...
创作与编辑共用一个文本工作区；正式格式只区分普通文本与配音脚本，听力、对话由用户在脚本要求中表达，AI 作为执行方式。
已实现的持久化绑定与固定输出取代旧文本流程中“同一个 Artifact 同时充当原稿与结果”的假设。
任意端口连线、多素材编译及 Scene 几何编辑仍在后续阶段。

2026-09-25 外部素材入口已收敛为一个导入动作：PNG/JPEG、UTF-8 文本/代码与 PCM S16 LE WAV
进入现有图像、文本、音频 Artifact。用户明确选择文件后，其字节或规范化图像作为不可变源版本接受；
后续 AI/编辑产物仍经候选审阅。自包含 HTML 保持文本源身份，可由用户显式打开离线、临时的
WebEngine 预览；预览不改变正式字节，不授予文件、网络、剪贴板或持久存储权限。其他代码文件仅作为
文本保存，不在项目内执行；导入 WAV 的实际生成或录制来源标记为未核实。可执行 JS 动画包、视频文件
和客户端 Agent 工作区需要各自的受控交付契约，不由普通导入入口隐含支持。
最终文件出口复用现有结果端点：已采用文本导出 UTF-8 文本或代码文件，栅格图像导出规范 PNG，音频沿用 WAV
导出。文件是当前正式版本的交付快照，不增加独立的图节点，也不将尚未采用的候选误当最终产物。
文本/代码导入保留原文件后缀作为作品名的一部分，可按支持的 UTF-8 文本扩展名导出原字节；
自包含 HTML 的原字节也可导出为 `.html`；视频导出须先定义可复现的时间轴和逐帧渲染契约。
Infer 可提供有版本、独立于普通 Responses 的 Agent 任务准入与受控文件工作区；Shape 仍负责创作意图、
候选审阅和接受，Infer 不拥有 Shape 的创作工作流。
首批 Agent 文件任务仅处理已接受的 UTF-8 文本或代码 Source：Shape 读取并验证项目内的正式字节，
只把字节副本、用户指令和受限的相对文件名交给 Infer。运行结果必须返回声明的单个 UTF-8 文件；
SDK 校验输出摘要，Shape 校验 Job 来源，再在原 Source 下显示临时候选。用户明确接受后，
才以精确输入 Revision 为来源创建独立的文本 Artifact；原稿不被覆盖。真实主机路径、
未接受草稿、任意文件访问和多文件工作区不属于这次入口。
...
当前桌面入口已能将剪贴板文字或图片直接接受为 Source；图片仍遵循既有栅格导入规则，持久化的是
规范化 PNG 修订。表中“原始粘贴/导入字节与规范化预览分离”是后续合同，不能把当前规范化修订
误称为原始载荷归档。
...
“新建内容”第一层区分自由文字与配音脚本，后者只有一个通用脚本入口；听力练习、多人对话是用户可写入要求的内容，不是单独入口或格式。
